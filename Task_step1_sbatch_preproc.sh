...
# Pre-processing script for "Repetition of Computer Security Warnings Results in Differential Repetition Suppression Effects as Revealed with Functional MRI"
# Data are available at https://openneuro.org/datasets/ds002363
# This script is written for a compute cluster using SLURM for job submission. Use the "submit" wrapper script to submit the job to the scheduler.
#
...
phaseArr=(rse1)					# Each PHASE of experiment, in order (TEST1 precedes TEST2 but followed STUDY). This is absolutely necessary - so put something here
