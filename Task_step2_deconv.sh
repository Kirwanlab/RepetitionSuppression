...
for i in sub*; do 
    ...
    if [ ! -f censor_${phase}_combined.1D ]; then
		runLength=()
	...
	#deconvolution 2, with regressor for novel stim
	# controls for stimulus size

	#run the regression analysis
	if [ ! -f ${phase}_4+tlrc.HEAD ]; then 
	...
	-stim_file  13 timing_files/stimSizeScaled.txt -stim_label 13 "stimSize" \
	...
	-stim_times 26 timing_files/novel.txt 'BLOCK(3,1)' -stim_label 26 "novel" \
	-noFDR -nofullf_atall \
	-x1D X.${phase}_4.xmat.1D \
	-xjpeg X.${phase}_4.jpg \
	-x1D_uncensored X.${phase}_4.nocensor.xmat.1D \
	-errts ${phase}_4_errts \
	-bucket ${phase}_4 \
	-jobs 12 \
	-GOFORIT 12
	fi
